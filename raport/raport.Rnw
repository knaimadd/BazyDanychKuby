...
ładna_kwota <- function(kwota) {
  return(if_else(kwota < 10^3, paste(kwota, "zł"), 
                 if_else(kwota < 10^6, paste(round(kwota*0.1^3, 2), "tyś. zł"), 
...
Na rysunku \ref{fig:fig_typ} przedstawione są dwa wykresy pudełkowe zysków, jeden dla samochodów, drugi dla motocykli. Większa mediana, wynosząca \Sexpr{round(max(miary$Q2), 2)}, jest dla pojazdów typu \Sexpr{Q2_max_grupa}. W drugiej grupie wynosi ona \Sexpr{round(min(miary$Q2), 2)} tyś. zł. 
Większy pierwszy kwartyl występuje w grupie typu \Sexpr{Q1_max_grupa}, wynosi on \Sexpr{round(max(miary$Q1), 2)} tyś. zł, w porównaniu dla grupy typu \Sexpr{miary$typ[miary$Q1==min(miary$Q1)]} jego wartość wynosi \Sexpr{round(min(miary$Q1), 2)} tyś. zł.
W przypadku kwartyla trzeciego większa wartość występuje w grupie typu \Sexpr{Q3_max_grupa} (wynosi \Sexpr{round(max(miary$Q3), 2)} tyś. zł). W drugiej grupie wynosi on \Sexpr{round(min(miary$Q3), 2)} tyś. zł.
Największy zysk przyniósł \Sexpr{max_max_grupa}, a wyniósł on \Sexpr{round(max(miary$max_wartość))} tyś. zł. 
Najmniejszy zysk natomiast przyniósł \Sexpr{min_min_grupa} i wyniósł on \Sexpr{round(min(miary$min_wartość))} tyś. zł\Sexpr{zdanie_dod}. 
...
Na rysunku \ref{fig:fig_wypadkowy} przedstawione są dwa wykresy pudełkowe zysków dla pojazdów powypadkowych i niepowypadkowych. Większa mediana, wynosząca \Sexpr{round(max(miary$Q2), 2)} tyś. zł, jest dla pojazdów \Sexpr{Q2_max_grupa}. W drugiej grupie wynosi ona \Sexpr{round(min(miary$Q2), 2)} tyś. zł. 
Większy pierwszy kwartyl występuje w grupie pojazdów \Sexpr{Q1_max_grupa}, wynosi on \Sexpr{round(max(miary$Q1), 2)} tyś. zł, w porównaniu dla grupy pojazdów \Sexpr{miary$czy_powypadkowy[miary$Q1==min(miary$Q1)]} jego wartość wynosi \Sexpr{round(min(miary$Q1))} tyś. zł.
Większa wartość trzeciego kwartylu występuje dla pojazdów \Sexpr{Q3_max_grupa} i wynosi \Sexpr{round(max(miary$Q3), 2)} tyś. zł. Dla pojazdów \Sexpr{miary$czy_powypadkowy[miary$Q3==min(miary$Q3)]} wynosi on \Sexpr{round(min(miary$Q3), 2)} tyś. zł.
Największy zysk przyniósł pojazd z grupy \Sexpr{max_max_grupa} i wyniósł on \Sexpr{round(max(miary$max_wartość), 2)} tyś. zł. 
Najmniejszy zysk natomiast przyniósł pojazd z grupy \Sexpr{min_min_grupa} i wyniósł on \Sexpr{round(min(miary$min_wartość), 2)} tyś. zł\Sexpr{zdanie_dod}. 
...
Na rysunku \ref{fig:fig_wypadkowy} przedstawione są wykresy pudełkowe zysków ze względu na pojemność silnika. Największa mediana, wynosząca \Sexpr{round(max(miary$Q2), 2)} tyś. zł, jest dla pojazdów o pojemności silnika \Sexpr{Q2_max_grupa} litra. Natomiast najmniej ona wynosi \Sexpr{round(min(miary$Q2), 2)} tyś. zł w grupie pojazdów o pojemności \Sexpr{Q2_min_grupa} litra. 
Największy pierwszy kwartyl występuje w grupie pojazdów o pojemności \Sexpr{Q1_max_grupa} litra, wynosi on \Sexpr{round(max(miary$Q1), 2)} tyś. zł, w porównaniu z pojazdami o pojemności \Sexpr{Q1_min_grupa} litra, dla których jego wartość jest najmniejsza i wynosi \Sexpr{round(min(miary$Q1), 2)} tyś. zł.
Największa wartość trzeciego kwartylu występuje dla pojazdów o pojemności \Sexpr{Q3_max_grupa} litra i wynosi \Sexpr{round(max(miary$Q3), 2)} tyś. zł. Dla pojazdów \Sexpr{Q3_min_grupa} litra wynosi on \Sexpr{round(min(miary$Q3), 2)} tyś. zł i jest to najmnijesza wartość w tych grupach.
Największy zysk przyniósł pojazd o pojemności silnika \Sexpr{max_max_grupa} litra i wyniósł on \Sexpr{round(max(miary$max_wartość), 2)} tyś. zł. 
Najmniejszy zysk natomiast przyniósł pojazd z pojemnością silnika \Sexpr{min_min_grupa} litra i wyniósł on \Sexpr{round(min(miary$min_wartość), 2)} tyś. zł.\Sexpr{zdanie_dod_max}\Sexpr{zdanie_dod_min}
...
df |> ggplot(aes(x=data, y=suma*0.1^3, , fill=format(data, "%m"))) + 
  ...
  labs(y = "Miesięczne wydatki (tyś. zł)")
...
df |> ggplot(aes(x=data, y=suma*0.1^3, , fill=format(data, "%m"))) + 
  geom_bar(stat = "identity") + 
  scale_fill_manual(name="Miesiąc",labels=unique(format(df$data, "%B")), values=hue_pal()(12)) + 
  labs(y = "Miesięczne przychody (tyś. zł)")
...
min_p_zdanie <- if_else(min_p < 100, paste(min_p, 'zł'), paste(round(min_p*0.1^3, 2), 'tyś. zł'))

min2_p <- min(miesiące[-1,]$suma)
min2_p_zdanie <- if_else(min2_p < 100, paste(min2_p, 'zł'), paste(round(min2_p*0.1^3, 2), 'tyś. zł'))
...
Wykres \ref{fig:fig_pracownicy} przedstawia miesięczne koszty wypłat pensji pracowników. Największy koszt warsztat odnotował w \Sexpr{miesiąc_odmiana(max_m)} \Sexpr{max_m} i wyniósł on \Sexpr{round(max_p*0.1^3, 2)} tyś. zł.\Sexpr{wniosek_max} 
Natomiast najmniejsze koszty wystąpiły w \Sexpr{miesiąc_odmiana(min_m)} \Sexpr{min_m}. Wyniosły one \Sexpr{min_p_zdanie}.\Sexpr{wniosek_min}. Poza tym miesiącem najmniej wypłacono pracownikom w \Sexpr{miesiąc_odmiana(min_m2)} \Sexpr{min_m2}. Była to kwota \Sexpr{min2_p_zdanie}
...
Największe miesięczne wpływy warsztatu ze sprzedaży pojazdów wynosiły \Sexpr{round(max_p*0.1^6, 2)} mln. zł. Wystąpiły one w \Sexpr{miesiąc_odmiana(max_m)} \Sexpr{max_m}. 
Drugie co do wielkości wpływy (w wysokości \Sexpr{round(max_p2*0.1^6, 2)} mln. zł) zostały odnotowane w \Sexpr{miesiąc_odmiana(max_m2)} \Sexpr{max_m2}. Są one \Sexpr{round(max_p / max_p2, 2)} razy mniejsze niż te największe zaobserwowane.
Najmniejsze miesięczne wpływy wyniosły \Sexpr{round(min_p*0.1^6, 2)} mln. zł, a wystąpiły one w \Sexpr{miesiąc_odmiana(min_m)} \Sexpr{min_m}.
Kolejnymi najmniejszymi są wpływy z \Sexpr{miesiąc_odmiana(min_m2)} \Sexpr{min_m2}. Są one wysokości \Sexpr{round(min_p2*0.1^6, 2)} mln. zł.
...
Najwięcej warsztat zarobił (\Sexpr{round(max_p*0.1^3, 2)} tyś. zł) w \Sexpr{miesiąc_odmiana(max_m)} \Sexpr{max_m}.
Natomiast najmniej w \Sexpr{miesiąc_odmiana(min_m)} \Sexpr{min_m}. Zyski wyniosły wtedy \Sexpr{round(min_p*0.1^3, 2)} tyś. zł.\Sexpr{wniosek}
...
miesiące |> ggplot(aes(x=data, y=bilans*0.1^3, color=format(data, "%m"))) + 
  ...
  scale_fill_manual(name="Miesiąc",labels=unique(format(miesiące$data, "%B")), values=hue_pal()(12)) + 
  labs(y = "Miesięczny bilans (tyś. zł)")
...
Wykres \ref{fig:fig_bilans_suma} przedstawia miesięczny stan finansowy warsztatu, gdyby zaczynał działalność bez środków pieniężnych. W pierwszym miesiącu działalności stan finansowy wyniósł \Sexpr{round(stan_początkowy*0.1^6, 2)} mln. zł, natomiast aktualnie wynosi on \Sexpr{round(stan_końcowy*0.1^6, 2)} mln. zł.\Sexpr{wniosek}
Największą ilością pieniędzy warsztat operował w \Sexpr{miesiąc_odmiana(max_m)} \Sexpr{max_m}, było to \Sexpr{round(max_p*0.1^6, 2)} mln. zł.
Natomiast najmniejszą w \Sexpr{miesiąc_odmiana(min_m)} \Sexpr{min_m}, a było to \Sexpr{round(min_p*0.1^6, 2)} mln. zł.
